...
local function checkCommand(command, updateType)
	...
	assert(constants[updateType .. "UpdateCommands"][command], "A command has to be registered if you want to check for it")
	...
	local assignee = settings[updateType .. "UpdateCommands"][command]
	...
	local ret
	local deltaPolicy = constants[updateType .. "UpdateCommands"][command]
